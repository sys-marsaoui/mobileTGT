...
public class FormationService {
    ...
    public ArrayList<Formation> parseFormations(String jsonText) {
        try {
            ...
            for (Map<String, Object> formationsJson : list) {
                ...
                int likes = formationsJson.get("likes") == null ? 0 : (int) Float.parseFloat(formationsJson.get("likes").toString());
                formations.add(new Formation(formation_id, id, titre_formation, description_formation, image, duree_formation, likes));
            }
    ...
    public boolean addFormation(Formation formation) {
        ...
        String url = Statics.BASE_URL + "/formation/newF" + "?titreFormation=" + formation.getTitre_formation() + "&descriptionFormation=" + formation.getDescription_formation() + "&image=" + formation.getImage() + "&dureeFormation=" + formation.getDuree_formation() + "&id=" + formation.getId() + "&likes=" + formation.getLikes();
        request.setUrl(url);
        request.addResponseListener(new ActionListener<NetworkEvent>() {
            @Override
            public void actionPerformed(NetworkEvent evt) {
    ...
    public boolean modifFormation(Formation formation) {
        System.out.println(formation);
        String url = Statics.BASE_URL + "/formation/" + formation.getFormation_id() + "/editF" + "?titreFormation=" + formation.getTitre_formation() + "&descriptionFormation=" + formation.getDescription_formation() + "&image=" + formation.getImage() + "&dureeFormation=" + formation.getDuree_formation() + "&id=" + formation.getId()+"&likes="+formation.getLikes();
        request.setUrl(url);
        request.addResponseListener(new ActionListener<NetworkEvent>() {
            @Override
            public void actionPerformed(NetworkEvent evt) {
    ...
    public ArrayList<Formation> TrierFormation(String val) {
        String url = Statics.BASE_URL + "/formation/api/trier/" + val;
    ...
    public ArrayList<Formation> RechercchFormation(String val) {
        String url = Statics.BASE_URL + "/formation/api/rechercheJson?keyWord=" + val;
